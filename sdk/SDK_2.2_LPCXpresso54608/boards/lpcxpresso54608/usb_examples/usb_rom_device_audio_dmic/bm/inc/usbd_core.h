...
typedef struct _USB_CORE_DESCS_T
{
    ...
    uint8_t *device_qualifier; /**< Pointer to USB device qualifier descriptor. For
                               ...
                               * be set to null (0).
                               */
    uint8_t *bos_descriptor;   /**< Pointer to USB BOS descriptor. For LPC Mode only.
             */
} USB_CORE_DESCS_T;
...
typedef struct USBD_API_INIT_PARAM
{
    ...
    *  callback will prevent the device from enumerating correctly or operate properly.
     ...
     *
     *  This callback is invoked by stack when get string descriptor request is received. If callback returns with pD
     * pointing to NULL,
     *  then the stack uses \em string_desc descriptor array provided during initialization to find the string. This
     * interface
     *  provides mechanism to override the stack handler for string desriptor and helps in handling dis-contiguous
     * string index descriptor requests.
     *  For example Windows host request string descriptor at index 0xEE to see if the devie supports Microsoft defined
     * OS descriptors.
     ...
     *  \param[out] pD  Double Pointer to string descriptor which is updated in the function. The pointed data should
     * persist
...
struct _USB_CORE_CTRL_T
{
    ...
    //  uint8_t *bos_descriptor; FIXME: Uncomment for ROM v19.1
    uint32_t ep_mask;
    uint32_t ep_halt;
    uint32_t ep_stall;
