...
ALIGN(64) static uint8_t data_buffer[MAX_SUBSTREAMS][((AUDIO_MAX_PKT_SZ + 0x3F) & ~0x3F) * NUM_DTDS];
void *const in_buff = data_buffer[SUBS_DMIC];
const uint32_t in_buff_sz = sizeof(data_buffer[SUBS_DMIC]);
...
ErrorCode_t usbd_init(void)
{
    ...
    if (ret == LPC_OK)
    {
        ...
        adc_param.maxp[SUBS_DMIC] = AUDIO_MAX_PKT_SZ;
        adc_param.ep_num[SUBS_DMIC] = USB_ADC_IN_EP;
